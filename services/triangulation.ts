...
export const triangulationService = {
    ...
    findBestStations: async (members: PartyMember[], meetingTime?: string): Promise<{ strategy: string, recommendations: any[] }> => {
        ...
        // ENRICHMENT: Fetch detailed TfL strings for the top 3 candidates (SAFEGUARD)
        const top3 = validScored.slice(0, 3);
        if (top3.length > 0) {
            console.log(`Enriching top ${top3.length} candidates with detailed route info...`);
            ...
            try {
                ...
                console.warn("Enrichment skipped or timed out:", e);
